...
def create_question_bank(bank):
    """创建题库"""
    try:
        response = requests.post(f'{BASE_URL}/question-banks', json=bank)
        if response.status_code == 200:
            result = response.json()
            print(f"✓ 创建题库成功: {bank['name']} (ID: {result['data']})")
            return result['data']
        elif response.status_code == 400:
            print(f"○ 题库已存在: {bank['name']}")
            return None
        else:
            print(f"✗ 创建题库失败 {bank['name']}: {response.status_code} - {response.text}")
            return None
    except Exception as e:
        print(f"✗ 创建题库异常 {bank['name']}: {e}")
        return None
...
def create_question(question):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✓ 创建题目成功: {content_preview} (ID: {result['data']})")
            return result['data']
        elif response.status_code == 400:
...
def main():
    ...
    for bank in QUESTION_BANKS:
        bank_id = create_question_bank(bank)
        if bank_id:
    ...
    # 统计数据
    try:
        response = requests.get(f'{BASE_URL}/question-banks')
        if response.status_code == 200:
            banks = response.json().get('data', [])
            print(f"题库总数: {len(banks)}")
