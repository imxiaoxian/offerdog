...
-- Vue相关题目 (bank_id=11)
(11, 4, 'Vue3的响应式原理是什么?Proxy和Object.defineProperty有什么区别?', 
...
(11, 4, 'Vue3的Composition API相比Options API有什么优势?', 
'Composition API的优势:
...
(11, 4, 'Vue的虚拟DOM原理是什么?diff算法是如何工作的?', 
...
(11, 4, 'Vue的生命周期钩子有哪些?Vue3和Vue2有什么区别?', 
'Vue3的生命周期钩子:
...
(11, 4, 'Vue的计算属性和侦听器有什么区别?分别在什么场景下使用?', 
'计算属性:
- 基于依赖缓存,依赖不变则不重新计算
- 用于同步计算,必须有返回值
- 适合用于模板渲染
...
-- React相关题目 (bank_id=12)
(12, 4, 'React的虚拟DOM原理是什么?和Vue有什么区别?', 
...
(12, 4, 'React Hooks的原理是什么?useEffect和useLayoutEffect有什么区别?', 
...
(12, 4, 'React的Context原理是什么?有什么优缺点?', 
...
-- JavaScript基础题目 (bank_id=13)
(13, 4, 'JavaScript的原型链是什么?如何实现继承?', 
...
(13, 4, 'JavaScript的闭包是什么?有什么用途和注意事项?', 
'闭包是函数记住其词法作用域的能力。
用途:
1. 数据封装和私有变量
2. 柯里化
...
(13, 4, 'JavaScript的事件循环机制是什么?宏任务和微任务有什么区别?', 
'事件循环机制:
1. 执行栈清空后执行微任务队列
2. 然后执行宏任务队列的一个任务
3. 重复这个过程
...
(13, 4, 'JavaScript的this指向规则是什么?如何改变this指向?', 
'this指向规则:
1. 全局上下文指向window
2. 对象方法指向调用对象
3. 构造函数指向新对象
...
(13, 4, 'JavaScript的深拷贝和浅拷贝有什么区别?如何实现深拷贝?', 
'浅拷贝只复制引用,深拷贝创建新对象。
深拷贝实现:
...
-- TypeScript题目 (bank_id=14)
(14, 4, 'TypeScript的泛型是什么?有什么用途?', 
'泛型是创建可重用组件的一种方式。
用途:
1. 函数重用
2. 数组类型安全
...
(14, 4, 'TypeScript的装饰器是什么?有哪些类型的装饰器?', 
...
(14, 4, 'TypeScript的交叉类型和联合类型有什么区别?', 
'交叉类型(&): 合并多个类型的属性
联合类型(|): 表示可以是多种类型之一
...
(14, 4, 'TypeScript的类型推断和类型断言有什么区别?', 
...
(15, 4, 'Vite和Webpack有什么区别?为什么Vite更快?', 
'Vite更快的原因:
1. 开发环境使用ESM,不需要打包
2. 预构建依赖
3. 按需编译
...
(15, 4, 'Monorepo和Multirepo有什么区别?什么时候使用Monorepo?', 
'Monorepo: 单仓库管理多个包
Multirepo: 每个包独立仓库
